...
class NormalizeService {
    ...
    normalize(data, customer, callback) {
        ...
        dao.find_all_fields(customer, function(fields) {
            ...
            var normalized_data = {
                ...
                company_segment: this.segment(this.getProperty(data, 'lead.custom_fields.Segmento', 0)),
                wip: this.works_in_progress(this.getProperty(data, 'lead.custom_fields.Sua empresa tem obras em andamento?', 0)),
                source_first_conv: this.source_of_convertion(this.getProperty(data, 'lead.first_conversion.conversion_origin.source', 0)),
                source_last_conv: this.source_of_convertion(this.getProperty(data, 'lead.last_conversion.conversion_origin.source', 0)),
                concern: this.biggest_concern(this.getProperty(data, 'lead.custom_fields.Qual sua maior preocupação hoje?', 0)),
    ...
    number_of_employees_in_office(data) {
        if (!data || data == '') {
            return 0;
        }
        if (data == '0 a 4') {
    ...
    looking_for_a_management_software(data) {
        if (!data || data == '') {
            return 9;
        }
        if (data == 'Não') {
